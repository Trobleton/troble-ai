...
class LLMWrapper:
    ...
    def _write_chat_history(self):
        logger.debug("Writing conversation history")

        chat_history_file = open(self.chat_history_filename, 'w', encoding='utf-8')
        json.dump({ "history": self.global_chat_history }, chat_history_file)
        chat_history_file.close()
    ...
    def _filter_markdown(self, text):
        ...
        filtered_text = re.sub(r"```.*?```", "", text, flags=re.DOTALL) # Remove code blocks
        # Remove inline code
